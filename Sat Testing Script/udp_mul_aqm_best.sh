...
TESTNAME_BASE="iperf3_d${DURATION}_best"
...
for AQM in "${AQM_LIST[@]}"; do
    echo "===== Starting Best Test with AQM: $AQM ====="
    ...
    nohup ./scripts/apply_tc_rules_best_${AQM}.sh > output_${AQM}.txt 2>&1 < /dev/null &
    tc_pid=$!
    echo "apply_tc_rules_best_${AQM}.sh running with PID $tc_pid"
    sleep 30
    ...
    base_dir="./data/udp_net_${TIMESTAMP}_best_${AQM}"
    mkdir -p "$base_dir"
...
echo "===== All AQM Best tests completed ====="
exit 0
