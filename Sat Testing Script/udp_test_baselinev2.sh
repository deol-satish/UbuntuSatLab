...
AQM_LIST=("fq_codel" "codel" "cake" "fq_pie" "pfifo")
for AQM in "${AQM_LIST[@]}"; do
    ...
    # Run apply_tc_rules_baseline.sh in the background
    # ./apply_tc_rules_baseline.sh &

    nohup ./apply_tc_rules_baseline.sh > output.txt 2>&1 < /dev/null &

    tc_pid=$!
    echo "apply_tc_rules_baseline.sh is running in the background with PID $tc_pid"
    sleep 20
    ...
    cleanup() {
        echo "Killing apply_tc_rules_baseline.sh with PID $tc_pid"
