...
# Run apply_tc_rules_best.sh in the background
# ./apply_tc_rules_best.sh &

nohup ./apply_tc_rules_best.sh > output.txt 2>&1 < /dev/null &

tc_pid=$!
echo "apply_tc_rules_best.sh is running in the background with PID $tc_pid"
sleep 20
...
base_dir="./data/udp_net_${timestamp}_best"
mkdir -p "$base_dir"
...
cleanup() {
    echo "Killing apply_tc_rules_best.sh with PID $tc_pid"
